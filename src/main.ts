...
import generateMaxTokensConfig from "./utils/generateMaxTokenConfig.js";
import getMaxTokensConfig from "./utils/getMaxTokensConfig.js";

export async function createCLI() {
  await generateConfig();
  await generateMaxTokensConfig();
  const program = new Command();
  let credentials: Map<string, string> =  getConfig();
  let tokens: Map<string, number> =  getMaxTokensConfig();
  let AI_MODEL = credentials.get("AI_MODEL");
  let API_KEY = credentials.get("API_KEY");
  let MAX_TOKENS=tokens.get("MAX_TOKENS");
  if(!MAX_TOKENS){
    await generateMaxTokensConfig();
  }
  if (!AI_MODEL) {
